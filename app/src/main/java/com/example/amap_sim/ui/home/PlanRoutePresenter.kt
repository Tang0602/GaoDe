...
import com.example.amap_sim.model.RouteSegment
...
class PlanRoutePresenter(
    ...
    // 生成首末班车警告信息
    private fun generateWarnings(busPath: com.amap.api.services.route.BusPath): List<com.example.amap_sim.model.RouteWarning> {
        val warnings = mutableListOf<com.example.amap_sim.model.RouteWarning>()
                ...
                if (firstBusTime.isNotEmpty()) {
                    try {
                        ...
                        if (currentMinutes >= firstBusMinutes - 30 && currentMinutes < firstBusMinutes) {
                            warnings.add(
                                com.example.amap_sim.model.RouteWarning(
                                    type = "first_bus",

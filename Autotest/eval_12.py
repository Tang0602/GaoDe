import subprocess
import json
import os
from datetime import datetime

def update_banu_selection_history(action_type, place_name):
    """更新巴奴火锅选择历史记录"""
    try:
        timestamp = int(datetime.now().timestamp() * 1000)
        selection_record = {
            "id": f"banu_{timestamp}",
            "action": action_type,
            "placeName": place_name,
            "timestamp": timestamp,
            "formattedTime": datetime.fromtimestamp(timestamp / 1000).strftime("%Y-%m-%d %H:%M:%S"),
            "category": "火锅",
            "rating": "4.5分",
            "price": "¥148/人",
            "success": True
        }
        
        history_file = os.path.join(os.path.dirname(__file__), '12_巴奴火锅选择历史.json')
        history_records = []
        
        if os.path.exists(history_file):
            try:
                with open(history_file, 'r', encoding='utf-8') as f:
                    history_records = json.load(f)
            except json.JSONDecodeError:
                history_records = []
        
        history_records.append(selection_record)
        
        with open(history_file, 'w', encoding='utf-8') as f:
            json.dump(history_records, f, ensure_ascii=False, indent=2)
        
        print(f"✅ 巴奴火锅选择历史记录已更新: {action_type} - {place_name}")
        return True
        
    except Exception as e:
        print(f"更新巴奴火锅选择历史失败: {e}")
        return False

def BanuHotpotSelectionCheck():
    """检测是否成功搜索并选择巴奴火锅"""
    try:
        result = subprocess.run(['adb', 'exec-out', 'uiautomator', 'dump', '/dev/stdout'], 
                              capture_output=True, text=True)
        
        if result.returncode == 0:
            ui_dump = result.stdout
            
            banu_indicators = [
                '巴奴毛肚火锅', '群光广场店', '4.5超棒', '¥148/人',
                '街道口火锅榜', '第5名', '营业中', '筒香小油条',
                '必点毛肚', '导航', '路线', '收藏'
            ]
            
            found_indicators = [indicator for indicator in banu_indicators if indicator in ui_dump]
            
            if found_indicators:
                print(f"✓ 在UI中找到巴奴火锅详情页元素: {', '.join(found_indicators)}")
                update_banu_selection_history("搜索并选择巴奴火锅", "巴奴毛肚火锅(群光广场店)")
                return True
            else:
                print("✗ 未在UI中找到巴奴火锅详情页元素")
                return False
        else:
            print(f"UI检测失败: {result.stderr}")
            return False
            
    except Exception as e:
        print(f"巴奴火锅选择检测失败: {e}")
        return False

if __name__ == "__main__":
    print("开始检测：搜索并选择巴奴火锅")
    result = BanuHotpotSelectionCheck()
    print(f"检测结果: {'通过' if result else '失败'}")
import subprocess
import json
import os
import base64
from datetime import datetime

def update_food_search_history(search_query):
    """Update food search history to device storage"""
    try:
        timestamp = int(datetime.now().timestamp() * 1000)
        search_record = {
            "id": f"search_{timestamp}",
            "query": search_query,
            "category": "Food",
            "timestamp": timestamp,
            "formattedTime": datetime.fromtimestamp(timestamp / 1000).strftime("%Y-%m-%d %H:%M:%S"),
            "resultCount": 4,
            "success": True
        }
        
        # Read existing history from device
        device_file_path = 'files/3_food_search_history.json'
        history_records = []
        
        try:
            result = subprocess.run(['adb', 'exec-out', 'run-as', 'com.example.GaoDe', 'cat', device_file_path],
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8', errors='ignore')
            if result.returncode == 0 and result.stdout and result.stdout.strip():
                try:
                    history_records = json.loads(result.stdout)
                except json.JSONDecodeError:
                    history_records = []
        except Exception:
            history_records = []
        
        history_records.append(search_record)
        
        # Create formatted temp file
        temp_file = os.path.join(os.path.dirname(__file__), 'temp_food_search.json')
        with open(temp_file, 'w', encoding='utf-8') as f:
            json.dump(history_records, f, ensure_ascii=False, indent=2)
        
        # Read and encode to base64
        with open(temp_file, 'r', encoding='utf-8') as f:
            json_content = f.read()
        
        json_bytes = json_content.encode('utf-8')
        json_b64 = base64.b64encode(json_bytes).decode('ascii')
        
        # Create file using base64
        create_result = subprocess.run(['adb', 'exec-out', 'run-as', 'com.example.GaoDe', 
                                      'sh', '-c', f'echo "{json_b64}" | base64 -d > {device_file_path}'],
                                     capture_output=True, text=True)
        
        os.remove(temp_file)
        
        if create_result.returncode == 0:
            verify_result = subprocess.run(['adb', 'exec-out', 'run-as', 'com.example.GaoDe', 
                                          'test', '-f', device_file_path],
                                         capture_output=True, text=True)
            
            if verify_result.returncode == 0:
                print(f"Food search history updated to device storage: {search_query}")
                return True
            else:
                print("File creation verification failed")
                return False
        else:
            print(f"Failed to create file: {create_result.stderr}")
            return False
        
    except Exception as e:
        print(f"Failed to update food search history: {e}")
        return False

def FoodSearchCheck():
    """Check if food search was successful"""
    try:
        # Check if food search result page elements exist in UI
        result = subprocess.run(['adb', 'exec-out', 'uiautomator', 'dump', '/dev/stdout'], 
                              capture_output=True, text=True, encoding='utf-8', errors='ignore')
        
        if result.returncode == 0:
            ui_dump = result.stdout
            
            if ui_dump is None:
                ui_dump = ""
            
            # Check food search result page specific UI elements
            food_indicators = [
                'Banu Hotpot', 'BBQ', 'McDonald', 'Restaurant',
                'Food', 'Hotpot', 'Barbecue', 'Fast Food', 'Average',
                '4.5', '4.7', '4.3', '4.6'
            ]
            
            found_indicators = [indicator for indicator in food_indicators if indicator in ui_dump]
            
            if found_indicators:
                print(f"SUCCESS: Found UI elements")
                if update_food_search_history("Food"):
                    return True
                else:
                    print("X Food search history update failed")
                    return False
            else:
                print("X Food search result elements not found in UI")
                return False
        else:
            print(f"UI detection failed: {result.stderr}")
            return False
            
    except Exception as e:
        print(f"Food search detection failed: {e}")
        return False

if __name__ == "__main__":
    print("Starting detection: Search Food")
    result = FoodSearchCheck()
    print(f"Detection result: {'PASS' if result else 'FAIL'}")